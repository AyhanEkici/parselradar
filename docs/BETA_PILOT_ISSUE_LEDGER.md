...
| PR-PROD-005 | 2026-05-25 | external.viewer.real | EXTERNAL_VIEWER | Conversational workflow | P1_FLOW_BLOCKER | https://parselradar.vercel.app/properties/new | User should be able to reach analysis intent through a guided conversation-first flow | MVP guided intake shell implemented with central composer, structured parcel fields, and next-step guidance to Yeni Mülk flow | MVP_SHELL_IMPLEMENTED_PENDING_USER_TEST | Product+Web UX | P2.CONVERSATIONAL-ANALYSIS-WORKFLOW-MVP-1 | Runtime user validation pending. Guidance-only boundaries and no-official-claim posture preserved. |
| PR-PROD-006 | 2026-05-25 | external.viewer.real | EXTERNAL_VIEWER | Navigation clarity | P2_PRODUCT_ISSUE | https://parselradar.vercel.app/properties/new | User should clearly understand whether tabs are required and what Yeni Mulk fills automatically | Dashboard/New Property/App shell copy now explains main path (create property -> add evidence/docs -> view report) and tabs as supporting pages | PARTIAL_HARDENED | Product+Web UX | P2.CONVERSATIONAL-ANALYSIS-WORKFLOW-MVP-1 | Further clarity and real-user validation still needed. |
