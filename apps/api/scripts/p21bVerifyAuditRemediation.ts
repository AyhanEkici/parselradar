...
async function main(): Promise<void> {
  ...
  const targetResults = targetFiles.map((file) => {
    ...
    return {
      ...
      triagedBacklogCount: countMatches(content, /P2_1B_TRIAGED_BACKLOG/g),
    };
  });
  ...
  const docExists = fs.existsSync("docs/P2_1B_AUDIT_REMEDIATION_SPRINT_2.md");
  const p21AuditExists = fs.existsSync("proof/p2-1-full-mvp-functional-audit-results.json");
  const p21aProofExists = fs.existsSync("proof/p2-1a-audit-remediation-results.json");
  ...
  const markdown = [
    "# P2.1B Audit Remediation Sprint 2 Results",
    "",
    `- Status: ${status}`,
    ...
    `- P2.1A proof exists: ${p21aProofExists}`,
  ...
  writeProofPair("proof/p2-1b-audit-remediation-results", payload, markdown);

  console.log(JSON.stringify({ status, proof: "proof/p2-1b-audit-remediation-results.Å½ÃŠ'" }, null, 2));
...
main().catch((error) => {
  ...
  writeProofPair(
    "proof/p2-1b-audit-remediation-results",
    payload,
    `# P2.1B Audit Remediation Sprint 2 Results\n\n- Status: FAIL\n- Detail: ${detail}\n`,
  );

  console.log(JSON.stringify({ status: "FAIL", proof: "proof/p2-1b-audit-remediation-results.json" }, null, 2));
  process.exitCode = 1;
});