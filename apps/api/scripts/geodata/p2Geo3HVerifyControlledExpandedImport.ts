...
async function main(): Promise<void> {
  if (!process.env.GEODATA_DATABASE_URL) {
    ...
    writeProofPair("proof/p2-geo-3h-expanded-staged-import-verification", payload, "# P2.GEO-3H Verification\n\n- Status: CONFIG_REQUIRED\n");
    console.log(JSON.stringify({ status: "CONFIG_REQUIRED", proof: "proof/p2-geo-3h-expanded-staged-import-verification.json" }, null, 2));
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    if (!run) {
      ...
      writeProofPair("proof/p2-geo-3h-expanded-staged-import-verification", payload, "# P2.GEO-3H Verification\n\n- Status: STAGED_IMPORT_REQUIRED\n");
      console.log(JSON.stringify({ status: "STAGED_IMPORT_REQUIRED", proof: "proof/p2-geo-3h-expanded-staged-import-verification.json" }, null, 2));
      process.exitCode = 1;
      return;
    }
    ...
    const featureTypes = Array.isArray(row.feature_types) ? row.feature_types.map(String).filter(Boolean) : [];

    const hasSettlement = featureTypes.includes("SETTLEMENT");
    const hasMainRoad = featureTypes.includes("MAIN_ROAD");
    const hasIndustrial = featureTypes.includes("INDUSTRIAL_OSB_CANDIDATE");
    ...
    writeProofPair(
      "proof/p2-geo-3h-expanded-staged-import-verification",
      payload,
      [
        "# P2.GEO-3H Expanded Staged Import Verification",
        "",
        `- Status: ${status}`,
    ...
    console.log(JSON.stringify({ status, proof: "proof/p2-geo-3h-expanded-staged-import-verification.json" }, null, 2));
    ...
    writeProofPair("proof/p2-geo-3h-expanded-staged-import-verification", payload, `# P2.GEO-3H Verification\n\n- Status: FAIL\n- Detail: ${detail}\n`);
    console.log(JSON.stringify({ status: "FAIL", proof: "proof/p2-geo-3h-expanded-staged-import-verification.json" }, null, 2));
    process.exitCode = 1;
  } finally {
