export function calculateWorkspaceSignals(input: {
  sharedAnalyses: Array<{ score?: number; opportunityScore?: number; freshnessScore?: number }>;
}) {
  ...
  const totals = input.sharedAnalyses.reduce<{
    score: number;
    opportunity: number;
    stale: number;
    ...
    (acc, row) => {
      ...
      if ((row.freshnessScore || 0) < 55) acc.stale += 1;
      if ((row.opportunityScore || 0) >= 70) acc.highPotential += 1;
      return acc;
    },
