...
export type AnalysisExplanationResult = {
  strengths: string[];
  risks: string[];
...
function deviationPercent(ratio: number) {
  return `${Math.round(ratio * 100)}%`;
}

export function generateAnalysisExplanation(input: ExplanationInput): AnalysisExplanationResult {
  ...
  if (input.developerFit === 'HIGH') strengths.push('Parcel shows strong developer suitability.');
  if (input.developerFit === 'MEDIUM') recommendations.push('Improve parcel and infrastructure evidence for stronger developer fit.');
  ...
  if (input.zoningPotential === 'HIGH') strengths.push('Zoning potential appears favorable.');
  ...
  if (input.liquiditySignal === 'HIGH') strengths.push('Liquidity outlook is favorable.');
  ...
  input.riskFlags.forEach((flag) => {
    if (!risks.includes(flag)) risks.push(flag);
  });
  ...
  const signal: AnalysisExplanationResult['signal'] =
    input.score >= 78 && input.riskClassification === 'LOW'
      ? 'STRONG'
  ...
  const summary =
    `${signal} profile. Score ${input.score}/100 with confidence ${input.confidence}%. ` +
