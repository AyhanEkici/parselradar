export function buildMacroStressScenario(input: {
  concentrationIndex: number;
  confidenceAverage: number;
  freshnessScoreAverage: number;
}) {
  const stressVulnerability = Number(
    (input.concentrationIndex * 0.55 + (100 - input.confidenceAverage) * 0.25 + (100 - input.freshnessScoreAverage) * 0.2).toFixed(2)
  );
