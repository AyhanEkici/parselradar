export function calculatePortfolioOpportunityScore(input: {
  analyses: Array<{ score?: number; opportunityScore?: number; freshnessScore?: number }>;
}) {
  ...
  const totals = input.analyses.reduce<{
    ...
    (acc, item) => {
      ...
      if ((item.freshnessScore || 0) < 55) acc.stale += 1;
      if ((item.opportunityScore || 0) >= 70) acc.highPotential += 1;
      return acc;
    },
