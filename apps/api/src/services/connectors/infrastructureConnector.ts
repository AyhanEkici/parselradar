...
type ConnectorInput = { city?: string; lastRefreshAt?: Date | string | null };
...
export function infrastructureConnector(input: ConnectorInput) {
  ...
  if (!endpoint) {
    ...
    if (ageMinutes > CONNECTOR_SCHEDULES.infrastructure.staleAfterMinutes) {
      status = 'STALE';
