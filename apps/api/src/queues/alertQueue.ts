...
export function enqueueAlert(payload: Record<string, unknown>) {
...
export function getAlertQueueState(): QueueState {
  const config = getRuntimeConfig();
  const failFlag = process.env.ALERT_QUEUE_FAIL === '1';
  const runFlag = process.env.ALERT_QUEUE_RUNNING === '1';
  const degradedFlag = process.env.ALERT_QUEUE_DEGRADED === '1';
  ...
  if (!config.redisConfigured) {
    ...
    state = 'FAILED';
    reason = 'Alert queue failure flag is set.';
  } else if (degradedFlag) {
    state = 'DEGRADED';
    reason = 'Alert queue degraded flag is set.';
  } else if (runFlag) {
    state = 'RUNNING';
    reason = 'Alert queue is marked as running.';
  }
  ...
  return {
    ...
    retries,
    failures,
    deadLetterReady: QUEUE_POLICIES.alert.deadLetterEnabled,
