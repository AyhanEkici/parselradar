...
export function enqueueAnalysis(payload: Record<string, unknown>) {
...
export function getAnalysisQueueState(): QueueState {
  const config = getRuntimeConfig();
  const failFlag = process.env.ANALYSIS_QUEUE_FAIL === '1';
  const runFlag = process.env.ANALYSIS_QUEUE_RUNNING === '1';
  const degradedFlag = process.env.ANALYSIS_QUEUE_DEGRADED === '1';
  ...
  if (!config.redisConfigured) {
    ...
    state = 'FAILED';
    reason = 'Analysis queue failure flag is set.';
  } else if (degradedFlag) {
    state = 'DEGRADED';
    reason = 'Analysis queue degraded flag is set.';
  } else if (runFlag) {
    state = 'RUNNING';
    reason = 'Analysis queue is marked as running.';
  }
  ...
  return {
    ...
    retries,
    failures,
    deadLetterReady: QUEUE_POLICIES.analysis.deadLetterEnabled,
