...
export function enqueueGeo(payload: Record<string, unknown>) {
...
export function getGeoQueueState(): QueueState {
  const config = getRuntimeConfig();
  const failFlag = process.env.GEO_QUEUE_FAIL === '1';
  const runFlag = process.env.GEO_QUEUE_RUNNING === '1';
  const degradedFlag = process.env.GEO_QUEUE_DEGRADED === '1';
  ...
  if (!config.redisConfigured) {
    ...
    state = 'FAILED';
    reason = 'Geo queue failure flag is set.';
  } else if (degradedFlag) {
    state = 'DEGRADED';
    reason = 'Geo queue degraded flag is set.';
  } else if (runFlag) {
    state = 'RUNNING';
    reason = 'Geo queue is marked as running.';
  }
  ...
  return {
    ...
    retries,
    failures,
    deadLetterReady: QUEUE_POLICIES.geo.deadLetterEnabled,
