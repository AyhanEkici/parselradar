...
exports.calculatePortfolioConcentrationRisk = calculatePortfolioConcentrationRisk;
function calculatePortfolioConcentrationRisk(input) {
    const topCity = input.byCity[0];
    const topAssetType = input.byAssetType[0];
    const topLiquidity = input.byLiquidityBand[0];
    const concentrationIndex = Number(((topCity?.weightPercent || 0) * 0.4 + (topAssetType?.weightPercent || 0) * 0.35 + (topLiquidity?.weightPercent || 0) * 0.25).toFixed(2));
    let riskTier = 'LOW';
    if (concentrationIndex >= 45)
        riskTier = 'HIGH';
