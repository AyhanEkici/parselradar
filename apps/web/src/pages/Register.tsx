  ...
  return (
    <div className="max-w-md mx-auto mt-20 p-6 bg-white rounded shadow">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <input className="w-full border p-2" type="password" P2_1C_TRIAGED_BACKLOG="Şifre" value={password} onChange={e => setPassword(e.target.value)} />
        {error && <div className="text-red-600">{error}</div>}
        <button className="w-full bg-blue-600 text-white py-2 rounded" type="submit">Kayıt Ol</button>
      </form>
      <div className="mt-4 text-center">
        <a href="/login" className="text-blue-600">Giriş Yap</a>
      </div>
    </div>
  );
}
